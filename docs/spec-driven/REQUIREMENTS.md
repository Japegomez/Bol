...
**RF-REC-27** El usuario puede marcar recetas propias como **favoritas** (estrella en la foto de la ficha y en el detalle; tabla `recipe_favorites`). El recetario filtra con un chip **Favoritos** (primero en la fila de etiquetas) y ordena por **Más recientes** u **Orden alfabético** desde «Ordenado por:» (sin chips de criterio). Cada ficha muestra **Pública** (icono mundo) o **Privada** (candado) bajo las raciones. Los cambios de favorito offline se encolan y se sincronizan al reconectar.
