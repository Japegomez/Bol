...
> **Fecha:** Agosto 2026
> **Estado:** F1–F15 + offline + modo cocina en producción de código. **v1.3.2+14** (hotfix): alergias personalizadas (`050`/`051`, `custom:…`), asistente con sustitutos de supermercado, fichas de altura uniforme (`RecipeCardTagsSlot`), fix `Navigator.maybeOf` al reconectar offline (Sentry MEALPLANER-G). **v1.3.1+13**: prompt de valoración diferido hasta uso real (RF-UX-02). **v1.3.0+12**: alergias en perfil (`048`), asistente consciente de alérgenos (RF-REC-28), timeout de sesión 7 días, orden estable de etiquetas. También: onboarding spotlight, offline Drift (SQLCipher), filtro multi-etiqueta, modo oscuro, asistente IA, hogar↔individual (024), share links (025 + token-gate), fork (026), remediación seguridad (`037`–`045`), favoritos (`046`).
...
| Autenticación | **Supabase Auth** + **Cloudflare Turnstile** | Email/contraseña + OAuth (Google y Sign in with Apple); CAPTCHA Turnstile si `TURNSTILE_SITE_KEY` está definido |
...
**RF-AUTH-09** La sesión se mantiene al minimizar o cambiar de app; expira cuando caduca el refresh token de Supabase (~1 semana), tras **≥7 días en background** (cierre automático al volver; alineado con el refresh token), o cuando el usuario cierra sesión manualmente.  
...
**RF-AUTH-12** El perfil puede marcarse como administrador de app (`profiles.is_admin`). Solo bootstrap/service role o un admin existente pueden cambiar ese flag (trigger `profiles_guard_admin`). Los payloads de cliente no envían `is_admin`. Los clientes **no** leen la columna `is_admin` (GRANT SELECT limitado, `043`); el flag propio se obtiene con la RPC `auth_is_admin()`.  
**RF-AUTH-13** Inicio de sesión, registro y recuperación de contraseña usan **Cloudflare Turnstile** cuando `TURNSTILE_SITE_KEY` está definido (`Env.hasTurnstile`). Si la clave está vacía, la app omite el CAPTCHA y muestra el aviso de configuración.  
...
**RF-AUTH-15** El usuario puede registrar sus **alergias e intolerancias** en el perfil (`profiles.allergens text[]`, migraciones `048`–`051`). Las claves predefinidas reutilizan las 10 etiquetas `*_free` del catálogo (`allergenTagKeys`): `gluten_free`, `lactose_free`, `dairy_free`, `egg_free`, `nut_free`, `peanut_free`, `soy_free`, `fish_free`, `shellfish_free`, `sugar_free`. Además puede añadir **alergias personalizadas** (`custom:<label>`: minúsculas, acentos permitidos, se elimina un prefijo «sin»/equivalente si el usuario lo escribe; máx. 10). La edición es una checklist en `/home/profile/allergies` (opción «Añadir…» al final) y se muestra un resumen de solo lectura en el perfil. El asistente de recetas las tiene en cuenta al generar una receta nueva (ver RF-REC-28).  
...
> **Nota de implementación — feedback y admin (migraciones `028`–`032`, `043`):** `profiles.is_admin` (boolean, default false); `user_feedback` (category `issue|feature|other`, status `pending|resolved|ignored`, RLS insert propio / select propio o admin / update solo admin). Función `auth_is_admin()` + trigger de guardia en INSERT/UPDATE. `anon`/`authenticated` no tienen SELECT sobre `is_admin`.
...
**RF-HH-02** El sistema genera un **código de invitación** único (alfanumérico, **8 caracteres** en códigos nuevos; los de 6 existentes siguen válidos) para cada hogar.  
**RF-HH-03** Cualquier usuario registrado puede unirse a un hogar introduciendo el código de invitación. Los intentos fallidos se limitan en servidor (`042`: intervalo mínimo, bloqueo tras 5 fallos en 5 minutos).  
...
> **Nota de implementación — RPCs (migraciones `006`, `037`, `042`–`044`):** `create_household(name)`, `join_household(code)` y `regenerate_invite_code(household_id)` son `SECURITY DEFINER` con `GRANT` a `authenticated` (no a `anon`). El INSERT de miembros solo pasa por esas RPCs (`037`). Códigos nuevos de 8 caracteres (sin ambiguos); `join_household` hace rate-limit y devuelve NULL si el código es inválido.
>
...
> **Nota de implementación — migración de plan (024 + 026 + `044`):** RPCs internas `merge_user_plans_into_household`, `snapshot_household_plans_to_user`, `rebuild_shopping_from_plans` (solo invocadas desde `create_household` / `join_household` / `leave_household`; **sin** `EXECUTE` para `anon` ni `authenticated`). `rebuild_shopping_from_plans` **preserva `is_checked`** de ítems automáticos que siguen existiendo (`plan_slot_id` + `ingredient_id`); ítems nuevos empiezan en `false`; manuales no se tocan. RLS `shares_household_with` para SELECT de recetas/ingredientes/pasos/nutrición/fotos entre co-miembros. Fork a recetario propio: RPC atómica `fork_recipe_into_my_book` (026, token obligatorio en enlace privado `039`) para hogar / pública / enlace activo.
...
**RF-REC-19** En las tarjetas del recetario y de exploración, las etiquetas de receta se muestran en una fila con **scroll horizontal** para no alargar la ficha verticalmente. Sin etiquetas, ambas fichas comparten la misma altura mínima (título + dos líneas de meta; foto cuadrada).  
...
**RF-REC-26** El asistente IA mitiga abuso y gasto **en servidor** (`check_and_increment_ai_usage`; el cliente **no** llama a Gemini). Límites (secrets `AI_ASSISTANT_*`, producción 13/08/2026): **20 llamadas/usuario/día** (receta, nutrición, `translate-recipe` y `moderate-image` comparten contador), cooldown **5 s**, tope **global 500/día**, tope **50/día por IP** (hash SHA-256; sin IP se omite el gate). Agotar global o IP → `service_at_capacity`. Complementan (no sustituyen) cuotas del proveedor `LLM_*`. No hay contabilidad de tokens ni coste en €; solo recuento de llamadas. `translate-titles` no entra en esta cuota.
**RF-REC-27** El usuario puede marcar como **favoritas** recetas propias y las compartidas por co-miembros del hogar (`recipe_favorites_insert_own` + `public.shares_household_with`; estrella en la foto de la ficha y en el detalle; tabla `recipe_favorites`). El recetario filtra con un chip **Favoritos** (primero en la fila de etiquetas) y ordena por **Más recientes** u **Orden alfabético** desde «Ordenado por:» (sin chips de criterio). Cada ficha muestra **Pública** (icono mundo) o **Privada** (candado) bajo las raciones. Los cambios de favorito offline se encolan y se sincronizan al reconectar.
**RF-REC-28** El asistente de IA es **consciente de las alergias** del usuario al generar una receta nueva (`generate_recipe`): el cliente envía `userAllergens` (del perfil, incluidas claves `custom:…`) en el body; la Edge Function **prioriza adaptar** con análogos habituales de supermercado (p. ej. masa/harina sin gluten, lácteos sin lactosa si solo aplica `lactose_free`, análogos vegetales si aplica `dairy_free`) y auto-aplica las etiquetas `*_free` predefinidas; devuelve `allergenAdjustments` (notas en español). Solo responde `allergen_conflict` (popup, no crea receta) cuando el alérgeno es **core** del plato y no hay sustituto razonable que preserve su identidad. Si hubo sustituciones/omisiones (o se infieren por etiquetas/`adaptado` en el título), la app muestra un diálogo listándolas **antes** de abrir el formulario. Las alergias no se aplican a los asistentes de nutrición ni de etiquetas sobre recetas existentes.
...
CREATE TABLE households (
  id           uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  name         text NOT NULL,
  invite_code  text UNIQUE NOT NULL,  -- 8 caracteres en códigos nuevos (042); 6 legacy válidos
...
- **RF-SOC-02** Pantalla de exploración (`/home/explore`) con buscador, filtro **multi-etiqueta** (AND, `list_public_recipes` con `p_tags text[]` y `@>`), scroll infinito (**10** recetas por página) y «Ordenado por: …» **seleccionable** (Más recientes / Mejor valoradas; sin chips de criterio) debajo de las etiquetas. Orden por defecto: fecha de creación descendente. Cada tarjeta muestra foto cuadrada alineada con el recetario (altura mínima compartida sin chips de categoría), título, **«Creada por» + autor en una fila bajo el título**, raciones y valoración, y **etiquetas en scroll horizontal**. Nombre del autor enlazado al perfil público, con tipografía destacada.
...
- **RF-SOC-08** El usuario puede **compartir una receta por enlace HTTPS** (WhatsApp u otras apps vía `share_plus`):
  ...
  - **Compartir:** enlace HTTPS + texto (sin adjuntar foto). Landing `share-landing` publica HTML de título en Storage (solo CTA **Instalar la app**; sin “Abrir en Böl”). Firebase Hosting redirige `/r/*`, `/p/*` y `/h/*`. Migraciones `025`, `032`, `034`, `036`–`045`.
