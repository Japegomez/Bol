...
**RF-REC-28** El asistente de IA es **consciente de las alergias** del usuario al generar una receta nueva (`generate_recipe`): el cliente envía `userAllergens` (del perfil) en el body; la Edge Function omite o sustituye ingredientes que contengan esos alérgenos, auto-aplica las etiquetas `*_free` correspondientes y devuelve `allergenAdjustments` (notas en español de cada cambio). Si la receta no se puede preparar sin un alérgeno core (p. ej. huevo en tortilla de patatas), responde `allergen_conflict` y la app muestra un popup sin crear la receta. Si hubo sustituciones/omisiones (o se infieren por etiquetas/`adaptado` en el título), la app muestra un diálogo listándolas **antes** de abrir el formulario. Las alergias no se aplican a los asistentes de nutrición ni de etiquetas sobre recetas existentes.
